...
run('vlfeat/toolbox/vl_setup')  % 将解压的一个vlfeat版本放在code文件夹中，并命名为vlfeat
...
%well.
num_train_per_cat = 100;  % 每个类别下最多有100个，不可以再增大
...
[train_image_paths, test_image_paths, train_labels, test_labels] = ...
    get_image_paths(data_path, categories, num_train_per_cat);  % 从data文件夹中的categories 分别读取文件，每个category文件夹取出来num_train_per_cat个
...
switch lower(FEATURE)    % 需要再前面指定FEATURE
    case 'tiny image'
    ...
    case 'bag of sift'
        ...
        if ~exist('vocab.mat', 'file')
            ...
            vocab_size = 600;     %Larger values will work better (to a point) but be slower to compute
            vocab = build_vocabulary(train_image_paths, vocab_size); % 从训练集中抽取特征，用k-means得到k个中心，也就是k个词汇
